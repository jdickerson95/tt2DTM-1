"""Backend functions related to inspecting and analyzing particle stack peaks."""
...
def core_inspect_peaks(
    ...
    -------
    dict[str, torch.Tensor]
        Dictionary containing the average surrounding correlation for all particles.
    """
    ...
    kwargs_per_device = construct_multi_gpu_inspect_peaks_kwargs(
...
# pylint: disable=too-many-locals
def construct_multi_gpu_inspect_peaks_kwargs(
    particle_stack_dft: torch.Tensor,
    template_dft: torch.Tensor,
    euler_angles: torch.Tensor,
    euler_angle_offsets: torch.Tensor,
...
def _core_inspect_peaks_single_gpu(
    result_dict: dict,
    device_id: int,
    particle_stack_dft: torch.Tensor,
    ...
    pbar_iter = tqdm.tqdm(
        range(num_particles),
        total=num_particles,
        desc=f"Inspecting peaks on device {device.index}...",
        leave=True,
        position=device_id,
        dynamic_ncols=True,
        unit="particle",
    ...
    #############################################################################
    ### Iterate over each particle in the stack to get the inspect statistics ###
    #############################################################################
    ...
    for i in pbar_iter:
        ...
        with torch.cuda.stream(stream):
            inspect_stats = _core_inspect_peaks_single_thread(
    ...
    # Extract the average surrounding correlation values
...
# pylint: disable=too-many-locals, too-many-statements
def _core_inspect_peaks_single_thread(
    ...
    -------
    dict[str, float]
        The inspect statistics for the particle.
    """
    ...
    tqdm_iter = tqdm.tqdm(
        range(num_batches),
        total=num_batches,
        desc=f"Inspecting particle {particle_index} on device {device_id}",
        leave=False,
    ...
    for i in tqdm_iter:
        ...
        # shape z_score is (num_Cs, num_defocus, num_orientations, y, x)
        ...
        if z_score.max() > max_z_score:
            ...
            # Get the specific z_score tensor for the best configuration
            best_z_score = z_score[px_idx, defocus_idx, angle_idx]
            
            # Create a mask to exclude the maximum pixel
            mask = torch.ones_like(best_z_score, dtype=torch.bool)
            mask[y_idx, x_idx] = False
            
            # Calculate average of all pixels except the maximum
            average_surrounding_cc = best_z_score[mask].mean().item()
